...
CLUSTER_NAME="${CLUSTER_NAME:-duihua-gateway-smoke}"
...
RELEASE="${RELEASE:-duihua-gateway}"
CHART="${CHART:-charts/duihua-gateway}"
...
dump_cluster_state() {
  ...
  kubectl logs -l "app.kubernetes.io/name=duihua-gateway,app.kubernetes.io/instance=${RELEASE}" --tail=200 2>/dev/null || true
}
...
for cmd in kind kubectl helm docker curl; do
  require_cmd "$cmd"
done
...
helm upgrade --install "${RELEASE}" "${CHART}" \
  ...
  --set responsesApiStore.enabled=false \
  --wait \
  --timeout "${HELM_TIMEOUT}"

log "waiting for duihua-gateway pod"
kubectl --context "${KUBECTL_CONTEXT}" wait --for=condition=ready pod \
  -l "app.kubernetes.io/name=duihua-gateway,app.kubernetes.io/instance=${RELEASE}" \
  --timeout=180s
...
log "waiting for local HTTP endpoint"
for attempt in $(seq 1 60); do
  if ! kill -0 "${PORT_FORWARD_PID}" 2>/dev/null; then
    cat "${PF_LOG}" >&2 || true
    dump_cluster_state
    echo "port-forward exited before HTTP became reachable" >&2
    exit 1
  fi
  ...
  if [[ "${attempt}" -eq 60 ]]; then
    cat "${PF_LOG}" >&2 || true
    dump_cluster_state
    echo "timed out waiting for HTTP on 127.0.0.1:${PORT}" >&2
    exit 1
  fi
  sleep 1
done
